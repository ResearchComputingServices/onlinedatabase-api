...
from onlinedatabase_api.providers.roles_provider import RoleProvider
...
@onlinedatabase_bp.route("/roles/count", methods=['GET'])
@crossdomain(origin='*')
@authentication
def get_role_count():
    return provider.get_count(Role)

@onlinedatabase_bp.route("/roles", methods=['GET'])
@crossdomain(origin='*')
@authentication
def get_role():
...
@onlinedatabase_bp.route("/roles", methods=['POST'])
@crossdomain(origin='*')
@authentication
def add_role():
    try:
...
@onlinedatabase_bp.route("/roles", methods=['PUT'])
@crossdomain(origin='*')
@authentication
def update_role():
    try:
...
@onlinedatabase_bp.route("/roles", methods=['DELETE'])
@crossdomain(origin='*')
@authentication
def delete_role():
    try:
