...
class UserKeycloak():
    ...
    def get_user_keycloak(self, username):
        url = oidc.client_secrets["keycloak_uri_onlinedatabase"]
            ...
            if token:
                ...
                if response.status_code >= 200 and response.status_code<=300:
                    ...
                    if len(data)>0:
                        ...
                        url = oidc.client_secrets["keycloak_uri_onlinedatabase"] + '/'+ user_keycloak_id + '/reset-password'
                        p = {"type": "password",
                             "temporary": "true",
                              "value": password
                            }
    ...
    def get_client_id(self, realm):
        ...
        id = ''
        url = oidc.client_secrets["keycloak_uri_onlinedatabase_base"] + '/clients/'
    ...
    def get_user_sessions(self, realm, first, max):
        ...
        if len(id)>0:
            if first and max:
                url = oidc.client_secrets["keycloak_uri_onlinedatabase_base"] + '/clients/' + id + '/user-sessions?first='+str(first)+'&max='+str(max)
            else:
                url = oidc.client_secrets["keycloak_uri_onlinedatabase_base"] + '/clients/' + id + '/user-sessions'
    ...
    def get_user_sessions_count(self, realm):
        ...
        if len(id) > 0:
            url = oidc.client_secrets["keycloak_uri_onlinedatabase_base"] + '/clients/' + id + '/session-count'
